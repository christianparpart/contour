...
namespace // {{{ helper
{
    void logGridText(Grid const& _grid, string const& _headline = "")
    {
        ...
        for (int row = 0; row < unbox<int>(_grid.historyLineCount()) + unbox<int>(_grid.screenSize().lines); ++row)
        {
            UNSCOPED_INFO(fmt::format(
                "{}: \"{}\" {}\n",
                row,
                _grid.renderTextLine(row - unbox<int>(_grid.historyLineCount()) + 1),
...
{
    auto line = Line(ColumnCount(5), "ABCDE"sv, Line::Flags::None);
    REQUIRE(!line.wrappable());
    REQUIRE(!line.wrapped());

    auto const reflowed = line.reflow(ColumnCount(3));
    CHECK(!line.wrapped());
    CHECK(*line.size() == 3);
...
{
    auto line = Line(ColumnCount(5), "ABCDE"sv, Line::Flags::Wrappable);
    REQUIRE(line.wrappable());
    REQUIRE(!line.wrapped());

    auto const reflowed = Line(line.reflow(ColumnCount(3)), line.inheritableFlags() | Line::Flags::Wrapped);
    CHECK(!line.wrapped());
    CHECK(*line.size() == 3);
    CHECK(line.toUtf8() == "ABC");
...
{
    auto line = Line(ColumnCount(5), Cell{}, Line::Flags::Wrappable);
    REQUIRE(!line.wrapped());
    REQUIRE(*line.size() == 5);
    REQUIRE(line.toUtf8() == "     ");

    auto const reflowed = Line(line.reflow(ColumnCount(3)), line.flags());
    CHECK(!line.wrapped());
    CHECK(*line.size() == 3);
    CHECK(line.toUtf8() == "   ");
    CHECK(*reflowed.size() == 0);
...
TEST_CASE("Grid.reflow.shrink.wrappable", "[grid]")
{
    ...
    (void) grid.resize(PageSize{LineCount(2), ColumnCount(3)}, Coordinate{1, 1}, false);
...
TEST_CASE("Grid.reflow.shrink", "[grid]")
{
    ...
    // 4x2
    (void) grid.resize(PageSize{LineCount(2), ColumnCount(4)}, Coordinate{1, 1}, false);
    logGridText(grid, "after resize");
    ...
    CHECK(grid.screenSize() == PageSize{LineCount(2), ColumnCount(4)});
    CHECK(grid.renderTextLine(1) == "abcd");
    ...
    CHECK(grid.screenSize() == PageSize{LineCount(2), ColumnCount(3)});
    CHECK(grid.renderTextLine(1) == "abc");
    CHECK(grid.renderTextLine(2) == "de ");

    // 2x2
    (void) grid.resize(PageSize{LineCount(2), ColumnCount(2)}, Coordinate{1, 1}, false);
    ...
    CHECK(grid.screenSize() == PageSize{LineCount(2), ColumnCount(2)});
...
TEST_CASE("Grid.reflow", "[grid]")
{
    ...
    SECTION("resize 2x2") {
        ...
        SECTION("regrow 3x2") {
            ...
            SECTION("regrow 5x2") {
                (void) grid.resize(PageSize{LineCount(2), ColumnCount(5)}, Coordinate{1, 1}, false);
                logGridText(grid, "after regrow 5x2");

                CHECK(grid.historyLineCount() == LineCount(0));
                CHECK(grid.screenSize() == PageSize{LineCount(2), ColumnCount(5)});
...
{
    auto grid = setupGrid5x2();
    REQUIRE(grid.screenSize() == PageSize{LineCount(2), ColumnCount(5)});
...
TEST_CASE("Grid.reflow.tripple", "[grid]")
{
    ...
    REQUIRE(grid.screenSize() == PageSize{LineCount(2), ColumnCount(2)});
    ...
    SECTION("grow from 2x2 to 8x2") {
        (void) grid.resize(PageSize{LineCount(2), ColumnCount(8)}, Coordinate{1, 1}, false);
        logGridText(grid, "after resize 3x2");

        CHECK(grid.historyLineCount() == LineCount(0));
        CHECK(grid.screenSize() == PageSize{LineCount(2), ColumnCount(8)});
    ...
    SECTION("grow from 2x2 to 3x2 to ... to 8x2") {
        ...
        REQUIRE(grid.screenSize() == PageSize{LineCount(2), ColumnCount(3)});
        ...
        REQUIRE(grid.screenSize() == PageSize{LineCount(2), ColumnCount(4)});
        ...
        // {{{ 7x2
        (void) grid.resize(PageSize{LineCount(2), ColumnCount(7)}, Coordinate{1, 1}, false);
        logGridText(grid, "after resize 7x2");

        REQUIRE(grid.historyLineCount() == LineCount(2));
        REQUIRE(grid.screenSize() == PageSize{LineCount(2), ColumnCount(7)});
        ...
        REQUIRE(grid.screenSize() == PageSize{LineCount(2), ColumnCount(8)});
